defmodule JwtFilterClaimsPlugTest do
  ...
  test "Filter passes any email address from domain" do
    ...
    conn =
      @the_plug.call(conn, [%{"email" => "^[_A-Za-z0-9-\\+]+(\\.[_A-Za-z0-9-]+)*@byteabyte.net$"}])
  ...
  test "Filter passes when email in token is last in config list" do
    ...
    conn =
      @the_plug.call(conn, [
        %{"email" => ["^someone@byteabyte.net$", "^alejandro.mezcua@byteabyte.net$"]}
      ])
  ...
  test "Filter fails when email in token is not in config list" do
    ...
    conn =
      @the_plug.call(conn, [%{"email" => ["^someone@byteabyte.net$", "^other@byteabyte.net$"]}])
